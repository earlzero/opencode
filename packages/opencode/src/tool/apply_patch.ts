      ...
      for (const hunk of hunks) {
        ...
        switch (hunk.type) {
          ...
          case "update": {
            ...
            const stats = yield* Effect.promise(() => fs.stat(filePath).catch(() => null))
            ...
            const oldContent = yield* Effect.promise(() => fs.readFile(filePath, "utf-8"))
          ...
          case "delete": {
            const contentToDelete = yield* Effect.promise(() =>
      ...
      for (const change of fileChanges) {
        ...
        switch (change.type) {
          case "add":
            ...
            yield* Effect.promise(() => fs.mkdir(path.dirname(change.filePath), { recursive: true }))
            yield* Effect.promise(() => fs.writeFile(change.filePath, change.newContent, "utf-8"))
          ...
          case "move":
            if (change.movePath) {
              // Create parent directories (recursive: true is safe on existing/root dirs)
              yield* Effect.promise(() => fs.mkdir(path.dirname(change.movePath!), { recursive: true }))
              yield* Effect.promise(() => fs.writeFile(change.movePath!, change.newContent, "utf-8"))
              yield* Effect.promise(() => fs.unlink(change.filePath))
        ...
        if (edited) {
          yield* Effect.promise(() => Format.file(edited))
